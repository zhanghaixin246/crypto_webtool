...
import (
	...
	"crypto_webtool/fabric/v2.2/internal-tools/cryptogen/ca"
	"crypto_webtool/fabric/v2.2/internal-tools/cryptogen/csp"
	"crypto_webtool/fabric/v2.2/internal-tools/cryptogen/msp"
...
func getConfig(osFile **os.File) (*Config, error) {
	var configData string
	genConfigFile = osFile
...
func Generate(osFile **os.File, outputDir *string,genUseRSA *bool) error {
	...
	for _, orgSpec := range config.PeerOrgs {
		err = renderOrgSpec(&orgSpec, "peer")
		if err != nil {
			return errors.New("Error processing peer configuration: " + err.Error())
			//fmt.Printf("Error processing peer configuration: %s", err)
			//os.Exit(-1)
		}
	...
	for _, orgSpec := range config.OrdererOrgs {
		err = renderOrgSpec(&orgSpec, "orderer")
		if err != nil {
			return errors.New("Error processing orderer configuration: " + err.Error())
			//fmt.Printf("Error processing orderer configuration: %s", err)
			//os.Exit(-1)
		}
