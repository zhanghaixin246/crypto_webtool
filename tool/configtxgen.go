...
import (
	...
	"crypto_webtool/fabric/v2.2/internal-tools/configtxgen/metadata"
...
func configtxgen() {
	...
	if outputBlock != "" || outputChannelCreateTx != "" || outputAnchorPeersUpdate != "" {
		...
		if configPath != "" {
			profileConfig, _ = genesisconfig.Load(profile, configPath)
		} else {
			profileConfig, _ = genesisconfig.Load(profile)
		}
	}
	...
	if channelCreateTxBaseProfile != "" {
		...
		if configPath != "" {
			baseProfile, _ = genesisconfig.Load(channelCreateTxBaseProfile, configPath)
		} else {
			baseProfile, _ = genesisconfig.Load(channelCreateTxBaseProfile)
		}
	}
