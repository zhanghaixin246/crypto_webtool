...
import (
	...
	"crypto_webtool/fabric/v2.2/common/viperutil"
	cf "crypto_webtool/fabric/v2.2/core/config"
...
func Load(profile string, configPaths ...string) (*Profile, error) {
	...
	uconf, err := cache.load(config, config.ConfigFileUsed())
	//res,_ := json.Marshal(uconf)
	//fmt.Println("uconf:",string(res))
	if err != nil {
	...
	if !ok {
		return nil, errors.New("Could not find profile: " + profile)
...
func (org *Organization) completeInitialization(configDir string) {
	...
	//fmt.Println("org :",string(res))
	translatePaths(configDir, org)
	//res1,_ := json.Marshal(org)
	//fmt.Println("new_org :",string(res1))
}
